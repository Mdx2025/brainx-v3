...
echo "🧠 BrainX V3 - Sistema de Restauración"
echo "======================================"
echo ""
...
if [ "$SKIP_DB" = false ]; then
    ...
    if command -v psql >/dev/null 2>&1; then
        DB_FILE="$EXTRACTED_DIR/brainx_v3_database.sql"
        ...
        if [ -f "$DB_FILE" ]; then
            ...
            if [ -z "${DATABASE_URL:-}" ]; then
                ...
                echo "   Formato: postgresql://user:pass@host:5432/brainx_v3"
                rm -rf "$BACKUP_DIR"
                exit 1
            fi
            ...
            if psql "$DATABASE_URL" -c "SELECT 1;" >/dev/null 2>&1; then
                ...
                DB_EXISTS=$(psql "$DATABASE_URL" -t -c "SELECT 1 FROM pg_database WHERE datname='brainx_v3';" 2>/dev/null | tr -d '[:space:]')
                
                if [ "$DB_EXISTS" = "1" ]; then
                    if [ "$FORCE" = false ]; then
                        print_warning "La base de datos brainx_v3 ya existe"
                    ...
                    print_info "Creando base de datos brainx_v3..."
                    psql "$DATABASE_URL" < "$DB_FILE" 2>/dev/null
...
# 2. Restaurar skill de BrainX V3
echo "📄 2/6 Restaurando skill BrainX V3..."
SKILL_BACKUP="$EXTRACTED_DIR/config/brainx-v3-skill"
if [ -d "$SKILL_BACKUP" ]; then
    if [ -d "${HOME}/.openclaw/skills/brainx-v3" ]; then
        if [ "$FORCE" = true ]; then
            rm -rf "${HOME}/.openclaw/skills/brainx-v3"
            cp -r "$SKILL_BACKUP" "${HOME}/.openclaw/skills/brainx-v3"
    ...
    for file in "$WRAPPERS_BACKUP"/*_wrapper.sh; do
        if [ -f "$file" ]; then
            ...
            if [ -d "$ws_hooks_dir" ]; then
                cp "$file" "$ws_hooks_dir/brainx-v3-wrapper.sh"
                chmod +x "$ws_hooks_dir/brainx-v3-wrapper.sh"
                echo "   ✅ Restaurado: $ws_name/hooks/brainx-v3-wrapper.sh"
            else
                mkdir -p "$ws_hooks_dir" 2>/dev/null || true
                cp "$file" "$ws_hooks_dir/brainx-v3-wrapper.sh" 2>/dev/null || true
                chmod +x "$ws_hooks_dir/brainx-v3-wrapper.sh" 2>/dev/null || true
            fi
        fi
    done
else
...
echo ""
echo "4. Verifica que BrainX V3 funciona:"
echo "   cd ~/.openclaw/skills/brainx-v3"
echo "   ./brainx health"
echo ""
